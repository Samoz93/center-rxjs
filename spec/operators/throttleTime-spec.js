    ...
    describe('default behavior { leading: true, trailing: false }', function () {
        it('should immediately emit the first value in each time window', function () {
            rxTest.run(function (_a) {
                ...
                var expected = '-a--------b-----c----|';
                var subs = '    ^--------------------!';
                var result = e1.pipe(operators_1.throttleTime(5, rxTest));
        ...
        it('should throttle events multiple times', function () {
            ...
            rxjs_1.concat(rxjs_1.timer(0, 1, rxTest).pipe(operators_1.take(3), operators_1.map(function (x) { return '1-' + x; })), rxjs_1.timer(8, 1, rxTest).pipe(operators_1.take(5), operators_1.map(function (x) { return '2-' + x; })))
        ...
        it('should simply mirror the source if values are not emitted often enough', function () {
            rxTest.run(function (_a) {
                ...
                var subs = '    ^--------------------!';
                var expected = '-a--------b-----c----|';
                expectObservable(e1.pipe(operators_1.throttleTime(5, rxTest))).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(subs);
            });
        });
        ...
        it('should raise error when source does not emit and raises error', function () {
            rxTest.run(function (_a) {
                ...
                var subs = '    ^----!';
                var expected = '-----#';
                expectObservable(e1.pipe(operators_1.throttleTime(10, rxTest))).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(subs);
            });
        });
        it('should handle an empty source', function () {
            rxTest.run(function (_a) {
                ...
                var subs = '    (^!)';
                var expected = '|';
                expectObservable(e1.pipe(operators_1.throttleTime(30, rxTest))).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(subs);
            });
        });
        ...
        it('should throttle and does not complete when source does not completes', function () {
            rxTest.run(function (_a) {
                ...
                var subs = '    ^------------------------------!';
                var expected = '-a-------------d----------------';
                expectObservable(e1.pipe(operators_1.throttleTime(5, rxTest)), unsub).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(subs);
            });
        });
        it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
            rxTest.run(function (_a) {
                ...
                var expected = '-a-------------d----------------';
                var unsub = '   -------------------------------!';
                var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.throttleTime(5, rxTest), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
    ...
    describe('throttleTime(fn, { leading: true, trailing: true })', function () {
        it('should immediately emit the first and last values in each time window', function () {
            rxTest.run(function (_a) {
                ...
                var result = e1.pipe(operators_1.throttleTime(t, rxTest, { leading: true, trailing: true }));
                expectObservable(result).toBe(expected);
