...
describe('Observable.prototype.concatMap', function () {
    ...
    it('should finalize before moving to the next observable', function () {
        var results = [];
        var create = function (n) { return rxjs_1.defer(function () {
            results.push("init " + n);
            return rxjs_1.of("next " + n).pipe(operators_1.delay(100, testScheduler), operators_1.finalize(function () {
                results.push("finalized " + n);
            }));
        }); };
        rxjs_1.of(1, 2, 3).pipe(operators_1.concatMap(function (n) { return create(n); })).subscribe({
    ...
    it('should map values to constant resolved promises and concatenate', function (done) {
        ...
        var results = [];
        source.pipe(operators_1.concatMap(project)).subscribe({ next: function (x) {
                results.push(x);
            }, error: function (err) {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.concatMap(function (value) { return rxjs_1.of(value); }), operators_1.take(3)).subscribe(function () { });
