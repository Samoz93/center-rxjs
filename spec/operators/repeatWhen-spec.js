...
describe('repeatWhen operator', function () {
    ...
    it('should not repeat when applying an empty notifier', function (done) {
        ...
        rxjs_1.of(1, 2).pipe(operators_1.map(function (n) {
            return n;
        }), operators_1.repeatWhen(function (notifications) { return rxjs_1.EMPTY; })).subscribe({ next: function (n) {
    ...
    it('should not error when applying an empty synchronous notifier', function () {
        ...
        rxjs_1.of(1, 2).pipe(operators_1.repeatWhen(function (notifications) { return rxjs_1.EMPTY; })).subscribe({ error: function (err) { return errors.push(err); } });
    ...
    it('should not error when applying a non-empty synchronous notifier', function () {
        ...
        rxjs_1.of(1, 2).pipe(operators_1.repeatWhen(function (notifications) { return rxjs_1.of(1); })).subscribe({ error: function (err) { return errors.push(err); } });
        rxjs_1.Observable.prototype.subscribe = originalSubscribe;
        chai_1.expect(errors).to.deep.equal([]);
    });
    ...
    it('should always teardown before starting the next cycle, even when synchronous', function () {
        var results = [];
        ...
        chai_1.expect(results).to.deep.equal([1, 2, 'teardown', 1, 2, 'teardown', 1, 2, 'teardown', 1, 2, 'complete', 'teardown']);
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.repeatWhen(function () { return rxjs_1.of(0); }), operators_1.take(3)).subscribe(function () { });
