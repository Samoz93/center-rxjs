...
describe('publishReplay operator', function () {
    ...
    it('should EMIT an error when the selector throws an exception', function () {
        ...
        var source = marble_testing_1.cold('--1-2---3-4---|');
        var published = source.pipe(operators_1.publishReplay(1, Infinity, selector));
        marble_testing_1.expectObservable(published).toBe('#', undefined, "It's broken");
    });
    ...
    it('should be referentially-transparent', function () {
        var source1 = marble_testing_1.cold('-1-2-3-4-5-|');
        var source1Subs = '^          !';
        var expected1 = '-1-2-3-4-5-|';
        var source2 = marble_testing_1.cold('-6-7-8-9-0-|');
        var source2Subs = '^          !';
        ...
        marble_testing_1.expectObservable(published1).toBe(expected1);
        marble_testing_1.expectSubscriptions(source1.subscriptions).toBe(source1Subs);
        marble_testing_1.expectObservable(published2).toBe(expected2);
        marble_testing_1.expectSubscriptions(source2.subscriptions).toBe(source2Subs);
        published1.connect();
        published2.connect();
    });
});
