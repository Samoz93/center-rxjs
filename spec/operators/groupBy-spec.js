...
describe('groupBy operator', function () {
    ...
    it('should group values', function (done) {
        ...
        rxjs_1.of(1, 2, 3).pipe(operators_1.groupBy(function (x) { return x % 2; })).subscribe({ next: function (g) {
    ...
    it('should group values with an element selector', function (done) {
        ...
        rxjs_1.of(1, 2, 3).pipe(operators_1.groupBy(function (x) { return x % 2; }, function (x) { return x + '!'; })).subscribe({ next: function (g) {
                var expectedGroup = expectedGroups.shift();
                chai_1.expect(g.key).to.equal(expectedGroup.key);
                g.subscribe(function (x) {
                    chai_1.expect(x).to.deep.equal(expectedGroup.values.shift());
    ...
    it('should group values with a duration selector', function () {
        ...
        var resultingGroups = [];
        rxjs_1.of(1, 2, 3, 4, 5, 6).pipe(operators_1.groupBy(function (x) { return x % 2; }, {
    ...
    it('should group values with a subject selector', function (done) {
        ...
        rxjs_1.of(1, 2, 3).pipe(operators_1.groupBy(function (x) { return x % 2; }, {
            connector: function () { return new rxjs_1.ReplaySubject(1); },
        }), operators_1.delay(5)).subscribe({ next: function (g) {
                var expectedGroup = expectedGroups.shift();
                chai_1.expect(g.key).to.equal(expectedGroup.key);
                g.subscribe(function (x) {
                    chai_1.expect(x).to.deep.equal(expectedGroup.values.shift());
    ...
    it('should allow an inner to be unsubscribed early but other inners continue', function () {
        ...
        var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }), operators_1.map(function (group) {
            var arr = [];
            var subscription = group.pipe(phonyMarbelize()).subscribe(function (value) {
                arr.push(value);
            });
            if (group.key === 'foo') {
    ...
    it('should dispose a durationSelector after closing the group', function () {
        ...
        var unsubscribedFrame = testing_1.TestScheduler
            ...
            .unsubscribedFrame;
        obs.pipe(operators_1.groupBy(function (val) { return val; }, {
    ...
    it('should return inners that when subscribed late exhibit hot behavior', function () {
        ...
        var result = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, function (val) { return val; }), operators_1.map(function (group) {
            ...
            rxTestScheduler.schedule(function () {
                group.pipe(phonyMarbelize()).subscribe(function (value) {
    ...
    it('should not error for late subscribed inners if outer is unsubscribed before inners are subscribed', function () {
        var source = marble_testing_1.hot('-----^----a----b-----a------b----a----b---#');
        var unsub = '                   !';
        var subjects = {
        ...
        var result = source.pipe(operators_1.groupBy(function (char) { return char; }), operators_1.tap({
            next: function (group) {
                rxTestScheduler.schedule(function () { return group.subscribe(subjects[group.key]); }, 1000);
            }
        }), operators_1.ignoreElements());
        marble_testing_1.expectObservable(result, unsub).toBe('-');
        marble_testing_1.expectObservable(subjects.a).toBe('-');
        marble_testing_1.expectObservable(subjects.b).toBe('-');
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.groupBy(function (value) { return value; }), operators_1.take(3)).subscribe(function () { });
        chai_1.expect(sideEffects).to.deep.equal([0, 1, 2]);
    });
});
function phonyMarbelize() {
    return function (source) { return source.pipe(operators_1.materialize(), operators_1.map(function (notification) {
        return {
