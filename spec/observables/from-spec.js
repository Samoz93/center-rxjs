...
describe('from', function () {
    ...
    var sources = [
        ...
        { name: 'readable-stream-like', createValue: function () { return new web_streams_polyfill_1.ReadableStream({
    ...
    var _loop_1 = function (source) {
        it("should accept " + source.name, function (done) {
            ...
            rxjs_1.from(source.createValue())
        ...
        it("should accept " + source.name + " and scheduler", function (done) {
            var nextInvoked = false;
            rxjs_1.from(source.createValue(), rxjs_1.asyncScheduler)
                .subscribe({ next: function (x) {
                    nextInvoked = true;
                    chai_1.expect(x).to.equal('x');
                }, error: function (x) {
        ...
        it("should accept a function that implements [Symbol.observable]", function (done) {
            ...
            var nextInvoked = false;
            rxjs_1.from(handler).pipe(operators_1.first()).subscribe({ next: function (x) {
                    nextInvoked = true;
                    chai_1.expect(x).to.equal('x');
                }, error: function (x) {
                    done(new Error('should not be called'));
